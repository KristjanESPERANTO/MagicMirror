...
describe("Translator", () => {
	...
	describe("load", () => {
		...
		it("should load translations", async () => {
			...
			dom.window.fetch = () => {
				return Promise.resolve({
					ok: true,
					json: () => Promise.resolve(translationTestData)
				});
			};
			await new Promise((resolve) => dom.window.onload = resolve);
		...
		it("should load translation fallbacks", async () => {
			...
			dom.window.fetch = () => {
				return Promise.resolve({
					ok: true,
					json: () => Promise.resolve(JSON.parse(fs.readFileSync(path.join(__dirname, "..", "..", "..", "tests", "mocks", file), "utf8")))
				});
			};
	...
	describe("loadCoreTranslations", () => {
		it("should load core translations and fallback", async () => {
			const dom = new JSDOM(
				`<script>var translations = {en: "http://localhost:3000/translations/translation_test.json"};</script>\
				<script src="file://${translatorJsPath}">`,
